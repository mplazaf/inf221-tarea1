...
ARCHIVOS = {
    "Merge Sort": "mergesort.csv",
    "Patience Sort": "patiencesort.csv",
    "sortArray": "sort.csv",
    "Quick Sort": "quicksort.csv",
}
    ...
    for tipo in tipos:
        for dominio in dominios:
            ...
            for algoritmo in ARCHIVOS:
                parte = resumen[resumen["algoritmo"] == algoritmo]
                if parte.empty:
                    continue
                plt.plot(
                    parte["n"],
            ...
            # se ven en escala logaritmica porque si no es mucho
            plt.xscale("log")
            plt.yscale("log")
...
def graficar_memoria(datos):
    ...
    for tipo in tipos:
        for dominio in dominios:
            filtro = datos[(datos["tipo"] == tipo)& (datos["dominio"] == dominio)]
            resumen = (filtro.groupby(["algoritmo", "n"], as_index=False)["memoria_kb"].mean())
            plt.figure(figsize=(9, 6))
            for algoritmo in ARCHIVOS:
                parte = resumen[resumen["algoritmo"] == algoritmo]
                if parte.empty:
